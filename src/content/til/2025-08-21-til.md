...
4. 4. 테스트도 정확히 어떤 테스트 조건에서 몇 번을 수행하여 어느 정도의 결과가 나오는지 반복적으로 테스트한 결과를 구체적으로 뽑아보려고 한다.
5. 5. 어떤 솔루션을 적용할 때 가장 중요한 것 중 하나는 비즈니스적으로 의미가 있는지 그리고 여러 가지 비용이 얼마나 소모되는지라고 생각해서 여러 상황을 놓고 생각해보려고 함
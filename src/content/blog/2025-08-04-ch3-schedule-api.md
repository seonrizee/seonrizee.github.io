...
- [1. 고민한 내용](#1-고민한-내용)
  - [1. @RequestBody와 DTO의 Lombok Annotation](#1-requestbody와-dto의-lombok-annotation)
  - [2. Schedule 엔티티 save() 후에 테이블에 저장되면서 MySQL이 자동 생성한 ID 값을 모르는데 저장이 되었는지 어떻게 확인할까?](#2-schedule-엔티티-save-후에-테이블에-저장되면서-mysql이-자동-생성한-id-값을-모르는데-저장이-되었는지-어떻게-확인할까)
  - [3. ID를 이용한 findById() 조회 결과가 null일 때 어떻게 처리해야 하는지?](#3-id를-이용한-findbyid-조회-결과가-null일-때-어떻게-처리해야-하는지)
  - [4. 필드가 조금 다른 경우 요청에 대하여 Request DTO의 재사용](#4-필드가-조금-다른-경우-요청에-대하여-request-dto의-재사용)
  - [5. 데이터베이스에서 수정한 시간을 바로 반영하여 반환 하려면?](#5-데이터베이스에서-수정한-시간을-바로-반영하여-반환-하려면)
  - [6. Service에서 delete 후 컨트롤러로 어떻게 성공 여부를 알려줘야할까?](#6-service에서-delete-후-컨트롤러로-어떻게-성공-여부를-알려줘야할까)
  - [7. 서비스에서 다른 서비스의 메서드가 필요할 때 서비스에 의존하는 게 나은가? 아니면 리포지토리에 의존하는 게 나은가?](#7-서비스에서-다른-서비스의-메서드가-필요할-때-서비스에-의존하는-게-나은가-아니면-리포지토리에-의존하는-게-나은가)
  - [8. 서비스 간 순환 참조 문제](#8-서비스-간-순환-참조-문제)
  - [9. 엔티티와 DTO, 어떻게 분리해야 할까? 그리고 변환 로직은 어디에서 관리해야 할까?](#9-엔티티와-dto-어떻게-분리해야-할까-그리고-변환-로직은-어디에서-관리해야-할까)
  - [10. 반복되는 API 응답 생성에 공통 DTO와 정적 팩토리 메서드 도입하기](#10-반복되는-api-응답-생성에-공통-dto와-정적-팩토리-메서드-도입하기)
...
## 1. @RequestBody와 DTO의 Lombok Annotation
...
## 2. Schedule 엔티티 save() 후에 테이블에 저장되면서 MySQL이 자동 생성한 ID 값을 모르는데 저장이 되었는지 어떻게 확인할까?
...
## 3. ID를 이용한 findById() 조회 결과가 null일 때 어떻게 처리해야 하는지?
...
## 4. 필드가 조금 다른 경우 요청에 대하여 Request DTO의 재사용
...
## 5. 데이터베이스에서 수정한 시간을 바로 반영하여 반환 하려면?
...
## 6. Service에서 delete 후 컨트롤러로 어떻게 성공 여부를 알려줘야할까?
...
## 7. 서비스에서 다른 서비스의 메서드가 필요할 때 서비스에 의존하는 게 나은가? 아니면 리포지토리에 의존하는 게 나은가?
...
## 9. 엔티티와 DTO, 어떻게 분리해야 할까? 그리고 변환 로직은 어디에서 관리해야 할까?
...
## 10. 반복되는 API 응답 생성에 공통 DTO와 정적 팩토리 메서드 도입하기

- 고민 내용: 10번 내용을 고민하면서 **정적 팩토리 메서드**에 대해 학습했습니다. 따라서, 활용할 수 있는 다른 곳을 찾아보았습니다.

...
- **예외 처리**: “0으로 나누기”를 시도하는 경우처럼 오류가 발생한 내용에 대해 출력을 하라는 요구 사항이 있었습니다. 그래서 일단은 Exception이 발생했을 때 프로그램이 바로 종료되지 않도록 미리 인지할 수 있는 문제는 예외 처리를 직접 해주는 것이 간단한 방법이라고 생각했습니다. 여기서는 try-catch를 이용하여 `exception`을 핸들링하지 않고 간단히 if와 콘솔 출력으로 처리했습니다.
